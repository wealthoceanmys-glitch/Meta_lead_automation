...
from app.whatsapp_webhook import is_whatsapp_webhook_payload, process_whatsapp_webhook_payload
...
app = FastAPI(title='Meta Leads + WhatsApp Automation', version='1.5.0', lifespan=lifespan)
...
@app.get('/')
def health():
    return {'status': 'ok', 'service': 'meta-leads-to-google-sheets-and-whatsapp'}
...
def _process_in_background(payload: Dict[str, Any]):
    try:
        if is_whatsapp_webhook_payload(payload):
            processed = process_whatsapp_webhook_payload(payload)
            print('Background WhatsApp webhook processing complete:', processed, flush=True)
            return

        processed = process_leadgen_payload(payload)
        print('Background processing complete. Lead IDs:', processed, flush=True)
    except Exception as exc:
        print('Background webhook processing error:', str(exc), flush=True)
...
@app.post('/test/whatsapp-status')
async def test_whatsapp_status(payload: Dict[str, Any]):
    """Manually test status webhook processing.
    ...
    result = process_whatsapp_webhook_payload(fake_payload)
