...
def reports_seminar(
    ...
    for lead in leads:
        ...
        buckets[(seminar_dt, seminar_label)].append({
            "status": status_value,
            "response": response_value,
        })

    rows = []
    for (seminar_dt, seminar_label), items in sorted(buckets.items(), key=lambda x: x[0][0] or _date.min):
        rows.append({
            "seminar_date": seminar_label,
            "total": len(items),
