...
class WhatsAppTemplate(Base):
    """
    Stores WhatsApp message templates submitted for Meta approval.
    Mirrors the WhatsApp Business API template object.
    """
    __tablename__ = "whatsapp_templates"

    id = Column(Integer, primary_key=True, index=True)
    # Meta-assigned template ID after submission
    meta_template_id = Column(String(100), nullable=True, index=True)
    name = Column(String(255), nullable=False, index=True)
    category = Column(String(80), nullable=False, default="MARKETING")  # MARKETING / UTILITY / AUTHENTICATION
    language = Column(String(20), nullable=False, default="en")
    status = Column(String(80), nullable=True)   # PENDING / APPROVED / REJECTED / PAUSED / DISABLED

    # ---- Header ----
    header_type = Column(String(20), nullable=True)   # TEXT / IMAGE / VIDEO / DOCUMENT / LOCATION / None
    header_text = Column(Text, nullable=True)          # for TEXT header
    header_media_handle = Column(String(500), nullable=True)  # upload handle for IMAGE/VIDEO/DOCUMENT

    # ---- Body ----
    body_text = Column(Text, nullable=False)
    # JSON list of variable examples: ["John", "Monday 10 AM", ...]
    ...
    # Full raw response from Meta on submit / refresh
