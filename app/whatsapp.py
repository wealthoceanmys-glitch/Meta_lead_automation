...
def build_registration_template_text(customer_name: str, seminar_date: str, seminar_time: str, arrival_time: str, venue: str) -> str:
    """Human-readable copy of the approved WhatsApp template, used for CRM chatbox history."""
...
def save_outgoing_template_message(db: Session, lead: Lead, wamid: str | None = None, status: str | None = None, raw: dict | None = None) -> WhatsAppMessage:
    """Save/update the automation template message so the CRM chatbox shows it as an outgoing bubble."""
    phone = clean_phone(lead.phone)
    _ensure_seminar_details(db, lead)
    message_id = wamid or lead.whatsapp_message_id or f"backfill:{lead.id}:template"
    message_status = status or lead.whatsapp_status or ("read" if lead.whatsapp_read else "delivered" if lead.whatsapp_delivered else "sent" if lead.whatsapp_sent else "accepted")
    body = build_registration_template_text(
        lead.full_name or "Customer",
        lead.seminar_date,
        lead.seminar_time,
        lead.arrival_time,
        lead.venue or settings.seminar_venue,
    )

    msg = None
    if message_id:
        msg = db.query(WhatsAppMessage).filter(WhatsAppMessage.wa_message_id == message_id).first()
    ...
    if msg:
        ...
        msg = WhatsAppMessage(
            ...
            raw=raw or {"source": "crm_template_history", "template": settings.whatsapp_template_name},
...
def send_template_for_lead(db: Session, lead: Lead, force: bool = False):
    ...
    if r.status_code in (200, 201):
        ...
        db.commit()
        db.refresh(lead)
        save_outgoing_template_message(db, lead, wamid=wamid, status=lead.whatsapp_status, raw=data)
        db.refresh(lead)
